...
test.describe('Customization Engine Tests', () => {
    ...
    test('Should apply coupon code and verify discount', async ({ page }) => {
        ...
        // Wait for the 'Checkout Now' button to appear (popup/modal)
        // Wait for the 'Checkout Now' button to appear (popup/modal)
        // Wait for the 'Checkout Now' button to appear (popup/modal)
        const checkoutBtn = page.getByText(/Checkout Now/i).first();
        await checkoutBtn.waitFor({ state: 'visible', timeout: 15000 });
        await checkoutBtn.click();

        // Wait for the Cart page content to load
        ...
        const couponInput = page.locator(`input[name="couponCode"][type="text"][placeholder="Enter Discount Code"]`);
        ...
        // Wait for discount to appear in the summary
        ...
        const discount = await discountLocator.textContent();
    ...
    test('test journey', async ({ page }) => {
        ...
        if (mainMenu) {
            ...
            // Wait for dropdown to appear/animate
            await page.waitForTimeout(500); // Small animation buffer usually acceptable if no specific element to wait for immediately, but better is to wait for Submenu
        ...
        const discount = await discountLocator2.textContent();
        ...
        // Replace fixed 60s wait with smart wait for loader to disappear
        console.log("Waiting for loader to disappear (up to 60s)...");
        ...
        try {
            ...
            console.log("Loader is STILL visible after 60s (or didn't appear).");
        }
    ...
    test('canada journey', async ({ page }) => {
